...
";
        ...
        {
            var lines = viewContent.Split(Environment.NewLine).ToArray();
            var cSharpCode = new StringBuilder();
            var supportedOperators = new[] {"for", "if", "foreach", "else"};
        ...
        private IView CompileAndInstance(string code, Assembly modelAssembly)
        {
            ...
            foreach (var assem in netStandardAssembly)
            {
                compilation =
                    compilation.AddReferences(MetadataReference.CreateFromFile(Assembly.Load(assem).Location));
            }
